 ...
 */
 ...
 list.shift();
 list.shift();
 var gameDir = list.shift();
 if (!gameDir) gameDir = "web/mygame/scenes/";
 fs = require('fs');
 vm = require('vm');
 path = require('path');
 load = function(file) {
 ...
 if (!list.length || (list.length == 1 && !list[0])) {
   ...
   if (fileExists(gameDir + "choicescript_stats.txt")) {
     list.push("choicescript_stats.txt");
   }
   if (fileExists(gameDir + "choicescript_screenshots.txt")) {
     list.push("choicescript_screenshots.txt");
   }
   if (fileExists(gameDir + "choicescript_upgrade.txt")) {
 ...
 Scene.prototype.verifySceneFile = function commandLineVerifySceneFile(sceneName) {
   ...
   try {
     verifyFileName(gameDir, fileName);
     if (fullGame) {
       addFile(fileName);
     }
   } catch (e) {
   ...
   (function(){
     ...
     for (var i = 0; i < list.length; i++) {
       try {
         ...
         verifyFileName(gameDir, fileName);
         var sceneText = slurpFile(gameDir+fileName, true /*throwOnError*/);
 ...
 var exitCode = (function(){
   for (var i = 0; i < list.length; i++) {
     print(list[i], { type: LOG_TYPES.PROGRESS, value: (i / list.length) * 100 });
     ...
     try {
       ...
       print(e.message, { type: LOG_TYPES.ERROR });
       if (isRhino) {
 ...
 if (!exitCode) {
   ...
   for (var sceneFile of fs.readdirSync(gameDir)) {
     ...
     if (nav.achievementList && nav.achievementList.length) {
       for (var i = 0; i < nav.achievementList.length; i++) {
         ...
         if (!nav.achieved[name]) {
           print("UNUSED achievement: " + name, { type: LOG_TYPES.WARNING });
         }
       }
     }
   })();
   ...
   if (!allLinesTested) print("SOME LINES UNTESTED", { type: LOG_TYPES.WARNING });
   if (typeof gameTitle === "undefined") {
     print("MISSING *TITLE COMMAND", { type: LOG_TYPES.WARNING });
   } else if (gameTitle.length > 30) {
     print("TITLE TOO LONG (" + gameTitle.length + " out of 30 characters): " + gameTitle, { type: LOG_TYPES.WARNING });
   }
   if (!authorIncluded) print("MISSING *AUTHOR COMMAND", { type: LOG_TYPES.WARNING });
   for (var i = 0; i < warnings.length; i++) {
     print(warnings[i], { type: LOG_TYPES.WARNING });
   }
   print("QUICKTEST PASSED", { type: LOG_TYPES.STATUS });
 }